...
int airodump_client::init(){
    ...
    const QString window_name("Device selector");
    ...
    devs = devs.filter(QRegExp("w+"));
    device = "";
    const QString const_a = QString("Devices (%1 available):").arg(devs.size());
    device = QInputDialog::getItem(NULL, window_name,
                                   const_a, devs, 0, false, &ok);
